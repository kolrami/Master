...
\begin{table}
	...
	\hline
	Hardware & Low & 6138 & 7 & 10937 & 1538\\
	Hardware & High & 15365 & 7 & 10560 & 1482\\
	Software & Low & 3115 & 7807 & 1136 & 3796\\
	Software & High & 11904 & 12944 & 1484 & 3435\\
	\hline
	\end{tabular}
\end{table}
As table \ref{tab:mbox_turn} indicates, a mailbox implemented in hardware
provides a significant speedup of around 1000 for hardware to hardware
communication. Furthermore, the communication is performed offside the
processor and, therefore, not influenced by the system's load and rather
jitter-free. Besides a speedup for hardware to hardware communication, a
hardware resources also provides speedup for software to hardware
...
thread, avoiding an additional thread switch.

For both hardware to software and software to software, a resource managed by
the operating system provides better results. The significant impact of the
load on the turnaround times of the hardware resources results from the
interrupt handling of the operating system. Under load, a long running thread
is, with high probability, executed when the interrupt occurs and scheduled
again, afterwards. Therefore, it needs additional time to schedule the
delegate thread and execute the appropriate get command. In contrary, for
software resources, the long running tasks are not interrupted and the blocked
evaluation threads gain a higher priority during waiting.
